...
param(
    [string]$TaskName = "WB_Order_Feed_Sync",
...
$jobScript = Join-Path $root "scripts\run_wb_order_feed.cmd"
$hiddenRunner = Join-Path $root "scripts\run_hidden.vbs"
$venvPython = Join-Path $root ".venv\Scripts\python.exe"
$jobFile = Join-Path $root "app\jobs\job_wb_order_feed.py"
...
$startAt = (Get-Date -Hour 0 -Minute 5 -Second 0)
...
$settings = New-ScheduledTaskSettingsSet -StartWhenAvailable -MultipleInstances IgnoreNew -ExecutionTimeLimit (New-TimeSpan -Hours 2) -RestartCount 2 -RestartInterval (New-TimeSpan -Minutes 2)
$description = "DataBase_MP: WB Order Feed, rolling 31-day current state and status history."
