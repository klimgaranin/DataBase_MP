...
$refreshScript = Join-Path $root "scripts\run_source_files_refresh.cmd"
$sourceJob = Join-Path $root "app\jobs\job_source_statistics.py"
$sheetsJob = Join-Path $root "app\jobs\job_sheets_source_files_export.py"
...
if ($PSCmdlet.ShouldProcess("$TaskPath$TaskName", "Register scheduled task")) {
    Register-ScheduledTask `
        ...
        -Trigger $trigger `
        -Settings $settings `
        -Description "DataBase_MP: checks internal Windows source files and exports DATA!Q:V/X:AC only when files changed." `
        -Force | Out-Null
