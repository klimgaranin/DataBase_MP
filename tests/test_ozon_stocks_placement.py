...
class PlacementAndLocalFileTests(unittest.TestCase):
    ...
    def test_powerquery_like_file_readers(self) -> None:
        with tempfile.TemporaryDirectory(ignore_cleanup_errors=True) as tmp:
            ...
            fill_sheet = workbook.create_sheet("Для заполнения")
            fill_sheet.append(["Артикул", "Спец-ия", "Менеджер", "Дата производства"])
            fill_sheet.append([100, "SPEC-1", "Анна", "03.07.2026"])
            ...
            self.assertEqual([row["Лист"] for row in specs], ["Для заполнения", "Пришло", "Пришло old"])
            self.assertEqual([row["Артикул"] for row in specs], [100, 200, 300])
            self.assertEqual(specs[0]["LOT"], "SPEC-1")
            self.assertEqual(specs[0]["Менеджер"], "Анна")
            self.assertEqual(specs[1]["LOT"], "L0124")
            self.assertEqual(specs[1]["Дата производства"], "04.07.2026")
            self.assertEqual(specs[2]["LOT"], "OLD")
            self.assertEqual(specs[2]["Менеджер"], "Ольга")
